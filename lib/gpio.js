...
function sleepSync(ms) {
  ...
  while (Date.now() - start < ms) {
    // Busy wait - minimal overhead for short delays
  }
}
    ...
    try {
      ...
      try {
        if (fs.existsSync(gpioPath)) {
          ...
          // Wait a moment for unexport to complete
          let retries = 10;
      ...
      if (usePolling) {
        ...
        }, 10); // Optimized: Reduced from 25ms to 10ms (100Hz) for precise detection
        
        // Store interval reference for cleanup
        coinInput.__pollInterval = pollInterval;
        
        console.log(`[GPIO] SUCCESS: GPIO ${sysPin} is now ACTIVE (polling mode).`);
      } else {
