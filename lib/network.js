...
async function restoreNetworkConfig() {
  ...
  try {
    const defaultWan = await getDefaultRouteInterface();
    if (defaultWan) {
      await execPromise(`ip link set dev ${defaultWan} nomaster`).catch(() => {});
    }
    // 1. Restore VLANs
    ...
    for (const bridge of bridges) {
      try {
        ...
        if (defaultWan) members = members.filter(m => String(m) !== String(defaultWan));
    ...
    for (const hotspot of hotspots) {
      try {
        if (defaultWan && String(hotspot.interface) === String(defaultWan)) continue;
...
async function autoProvisionNetwork() {
  ...
  try {
    ...
    const wanName = defaultWan || classified.wanName;
