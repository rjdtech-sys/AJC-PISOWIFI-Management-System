...
async function initFirewall() {
  ...
  try {
          ...
          if (expiredUsers && expiredUsers.length > 0) {
            ...
            for (const user of expiredUsers) {
              ...
              if (ip && isValidIp(ip)) {
                ...
                // Redirect DNS to local server so expired users resolve to error page
                await execPromise(`iptables -t nat -I PREROUTING 1 -s ${ip} -i ppp+ -p udp --dport 53 -j REDIRECT --to-ports 53`).catch(() => {});
                await execPromise(`iptables -t nat -I PREROUTING 1 -s ${ip} -i ppp+ -p tcp --dport 53 -j REDIRECT --to-ports 53`).catch(() => {});
                // Redirect HTTP to local portal (serves error.html via Express middleware)
    ...
    // 4.0 PPPoE expired user rules are already inserted with -I (highest priority)
    // Do NOT add a blanket ppp+ ACCEPT in PREROUTING - it would override expired user redirects
    // The FORWARD chain still allows ppp+ traffic (see below)
...
async function syncPPPoESecrets() {
  ...
  try {
    ...
    // Load server's main IP pool for dynamic assignment (instead of '*')
    const serverConfig = await db.get('SELECT local_ip, ip_pool_start, ip_pool_end FROM pppoe_server WHERE enabled = 1 LIMIT 1').catch(() => null);
    const mainPoolStartInt = serverConfig?.ip_pool_start ? ipToInt(String(serverConfig.ip_pool_start).trim()) : null;
    const mainPoolEndInt = serverConfig?.ip_pool_end ? ipToInt(String(serverConfig.ip_pool_end).trim()) : null;
    ...
    // Allocate a static IP from the server's main pool (avoids '*' which causes wrong-subnet assignment)
    const allocateMainPoolIp = async () => {
      if (mainPoolStartInt === null || mainPoolEndInt === null) return null;
      const start = Math.min(mainPoolStartInt, mainPoolEndInt);
      const end = Math.max(mainPoolStartInt, mainPoolEndInt);
      for (let i = start; i <= end; i++) {
        const ip = intToIp(i);
        // Skip server's own IP and already-used IPs
        if (ip === String(serverConfig?.local_ip || '').trim()) continue;
        if (isInExpiredPool(ip)) continue;
        if (!usedIps.has(ip)) {
          usedIps.add(ip);
          return ip;
        }
    ...
    for (const user of users) {
      ...
      if (isExpired && poolStartInt !== null && poolEndInt !== null) {
        ...
        if (assigned) {
          ...
          // No expired pool IPs available - try main pool
          const mainIp = await allocateMainPoolIp();
          if (mainIp) {
            ipField = mainIp;
            await db.run('UPDATE pppoe_users SET ip_address = ? WHERE id = ?', [mainIp, user.id]).catch(() => {});
          } else {
            ipField = '*';
          }
        }
        ...
        // Assign a static IP from the server's main pool to ensure correct subnet
        console.log(`[PPPoE-Server] User "${username}" is expired - assigning main pool IP for portal redirect`);
        const currentIp = String(user.ip_address || '').trim();
        if (currentIp && isValidIp(currentIp) && !isInExpiredPool(currentIp)) {
          ipField = currentIp;
        } else {
          const mainIp = await allocateMainPoolIp();
          if (mainIp) {
        ...
        if (staticIp && isValidIp(staticIp) && !isInExpiredPool(staticIp)) {
          ...
          // IP is in expired pool - allocate a new one from main pool
          ...
          // No static IP set - allocate from main pool instead of using '*'
          const mainIp = await allocateMainPoolIp();
          if (mainIp) {
            ipField = mainIp;
            await db.run('UPDATE pppoe_users SET ip_address = ? WHERE id = ?', [mainIp, user.id]).catch(() => {});
