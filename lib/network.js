...
async function getLanInterface() {
  ...
  const anyWifi = interfaces.find(i => i.type === 'wifi' && i.status === 'up') || interfaces.find(i => (i.name || '').startsWith('wl'));
  if (anyWifi) return anyWifi.name;

  const anyUp = interfaces.find(i => i.status === 'up' && i.name !== wanName && i.name !== 'lo' && !String(i.name || '').startsWith('ifb'));
  return anyUp ? anyUp.name : 'br-lan';
}
