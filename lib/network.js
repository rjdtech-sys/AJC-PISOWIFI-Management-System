...
async function configureWanDhcp(iface, isVlan = false) {
  ...
  try {
    ...
    // Start DHCP client (dhclient or dhcpcd)
    ...
    console.log(`[WAN] DHCP configured on ${dev}`);
    return { success: true };
  } catch (e) {
...
async function applyWanConfig(wan) {
  ...
  switch (type) {
    case 'dhcp':
      return configureWanDhcp(name, !!is_vlan);
    case 'static':
      return configureWanStatic(name, config);
    case 'pppoe':
      return configureWanPppoe(name, config);
    default:
