  ...
  try {
    ...
    try {
      ...
      if (pool && pool.ip_pool_start && pool.ip_pool_end) {
        ...
        await execPromise(`iptables -t nat -I PREROUTING 1 -i ppp+ -m iprange --src-range ${start}-${end} -p tcp --dport 80 -j REDIRECT --to-ports 80`).catch(() => {});
        await execPromise(`iptables -I FORWARD 1 -i ppp+ -m iprange --src-range ${start}-${end} -j DROP`).catch(() => {});
...
async function syncPPPoESecrets() {
  ...
  try {
    ...
    for (const user of users) {
      ...
      let ipField = '*';
      const expiresAt = String(user.expires_at || '').trim();
