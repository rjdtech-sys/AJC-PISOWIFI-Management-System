  ...
  try {
    ...
    try {
      ...
      if (pool && pool.ip_pool_start && pool.ip_pool_end) {
        ...
        const iprangeRuleNat = `iptables -t nat -I PREROUTING 1 -i ppp+ -m iprange --src-range ${start}-${end} -p tcp --dport 80 -j REDIRECT --to-ports 80`;
        const iprangeRuleFwd = `iptables -I FORWARD 1 -i ppp+ -m iprange --src-range ${start}-${end} -j DROP`;

        let applied = false;
        try {
          await execPromise(iprangeRuleNat);
          await execPromise(iprangeRuleFwd);
          applied = true;
        } catch (e) {
        ...
        if (!applied) {
          ...
          if (startParts.length === 4 && endParts.length === 4 && startParts.slice(0, 3).join('.') === endParts.slice(0, 3).join('.')) {
            ...
            await execPromise(`iptables -t nat -I PREROUTING 1 -i ppp+ -s ${cidr} -p tcp --dport 80 -j REDIRECT --to-ports 80`).catch(() => {});
            await execPromise(`iptables -I FORWARD 1 -i ppp+ -s ${cidr} -j DROP`).catch(() => {});
          }
        }
...
async function getPPPoESessions() {
  try {
    ...
    for (const pppIface of pppInterfaces) {
      ...
      const ip = (addr?.peer || addr?.local) || 'N/A';
      ...
      try {
        ...
        if (fs.existsSync(logPath)) {
          ...
          const lines = logs.split('\n').filter(l => l.includes(ifname) && /user/i.test(l));
          for (let i = lines.length - 1; i >= 0; i--) {
            const m = lines[i].match(/user\s+['"]?([^'"\s]+)['"]?/i);
            if (m && m[1]) {
              username = m[1];
              break;
            }
...
async function addPPPoEUser(username, password, billing_profile_id = null, expires_at = null) {
  console.log(`[PPPoE-Server] Adding user: ${username}`);
  
  try {
    // 1. Add to database
    const result = await db.run(
      'INSERT INTO pppoe_users (username, password, enabled, billing_profile_id, expires_at) VALUES (?, ?, 1, ?, ?)',
      [username, password, billing_profile_id, normalizeExpiresAt(expires_at)]
    );
...
async function updatePPPoEUser(userId, updates) {
  try {
    const { username, password, enabled, billing_profile_id, expires_at } = updates;
...
async function disconnectPPPoEUser(username) {
  ...
  try {
    ...
    const ifacePidPairs = getPppIfacePidMap();
    const matchedIfaces = [];
    for (const { iface, pid } of ifacePidPairs) {
      const u = await getUsernameForPidFromLog(pid);
      if (u && String(u).trim().toLowerCase() === user.toLowerCase()) {
        matchedIfaces.push({ iface, pid });
      }
    }
    ...
    try {
      ...
      if (fs.existsSync(logPath)) {
        const logs = fs.readFileSync(logPath, 'utf8');
        const lines = logs.split('\n');
        const userRe = new RegExp(`\\b${user.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&')}\\b`, 'i');
        for (let i = lines.length - 1; i >= 0; i--) {
          const line = lines[i];
          if (!line) continue;
          if (!userRe.test(line)) continue;
          const m = line.match(/\b(ppp\d+)\b/);
          if (m && m[1]) {
            iface = m[1];
            break;
          }
    ...
    if (matchedIfaces.length) {
      console.log(`[PPPoE-KICK] Matched via pidfiles+log: ${matchedIfaces.map(x => `${x.iface}(pid=${x.pid})`).join(', ')}`);
      for (const { iface: ifn, pid } of matchedIfaces) {
        try {
