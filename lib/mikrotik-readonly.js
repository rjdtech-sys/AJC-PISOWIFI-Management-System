...
async function createSchedulerForSecret(router, username, expiredProfile, duedate) {
  ...
  // Convert duedate to MikroTik format (e.g., "2024-05-15 00:00:00")
  ...
  const mikrotikDate = `${year}-${month}-${day} ${hours}:${minutes}:${seconds}`;
  ...
  const schedulerPayload = {
    ...
    'start-time': '00:00:00',
    'run-count': '1',
    'on-event': `/ppp secret set [find name="${username}"] profile=${expiredProfile}`
  };
  
  return await restPostJson(req, '/rest/system/scheduler', schedulerPayload, 10000);
}
...
async function createSchedulerForSecretViaAPI(client, username, expiredProfile, duedate) {
  ...
  return await client.menu('/system scheduler').call('add', schedulerPayload);
}
...
async function createSecret(routerId, secretData) {
  ...
  const { name, password: secretPassword, billing_plan_id, pppoe_profile, expired_profile, service, disabled, comment, duedate } = secretData;
  ...
  if (connectionType === 'rest') {
    ...
    const secretPayload = { name, password: secretPassword, profile: pppoe_profile, service, disabled, comment };
    const result = await restPostJson(req, '/rest/ppp/secret', secretPayload, 10000);
    ...
    if (duedate && expired_profile && name) {
      try {
        ...
        console.error('Failed to create scheduler:', schedulerErr);
...
async function updateSecret(routerId, secretId, secretData) {
  ...
  if (connectionType === 'rest') {
    ...
    return await restPostJson(req, '/rest/ppp/secret/set', { '.id': secretId, ...secretData }, 10000);
  } else {
    return await withRouterClient(router, async (client) => {
...
async function deleteSecret(routerId, secretId) {
  ...
  if (connectionType === 'rest') {
    ...
    return await restPostJson(req, '/rest/ppp/secret/remove', { '.id': secretId }, 10000);
  } else {
    return await withRouterClient(router, async (client) => {
