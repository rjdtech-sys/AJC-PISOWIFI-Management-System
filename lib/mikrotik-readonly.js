...
async function createSecret(routerId, secretData) {
  ...
  const { name, password, billing_plan_id, pppoe_profile, expired_profile, service, disabled, comment, duedate } = secretData;
  ...
  if (connectionType === 'rest') {
    ...
    const secretPayload = { name, password, profile: pppoe_profile, service, disabled, comment };
