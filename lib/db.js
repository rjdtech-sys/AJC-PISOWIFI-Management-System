...
async function init() {
  ...
  try {
    await run("ALTER TABLE rates ADD COLUMN expiration_hours INTEGER");
  } catch (e) {}
  try {
    await run("ALTER TABLE rates ADD COLUMN is_pausable INTEGER DEFAULT 1");
  } catch (e) {}
  ...
  try {
    await run("ALTER TABLE sessions ADD COLUMN pausable INTEGER DEFAULT 1");
  } catch (e) {}
  ...
  // Migration: ensure credit columns exist on wifi_devices for older installations
  try {
    const columns = await all("PRAGMA table_info('wifi_devices')");
    const names = columns.map(c => c.name);
    if (!names.includes('credit_pesos')) {
      await run("ALTER TABLE wifi_devices ADD COLUMN credit_pesos INTEGER DEFAULT 0");
    }
    if (!names.includes('credit_minutes')) {
      await run("ALTER TABLE wifi_devices ADD COLUMN credit_minutes INTEGER DEFAULT 0");
    }
  } catch (e) {
    console.log('[DB] wifi_devices credit columns migration error:', e.message);
  }
  ...
  try {
    ...
    if (!sessionCols.some(row => row.name === 'expired_at')) {
      console.log('[DB] Migration: Adding expired_at to sessions');
      await run("ALTER TABLE sessions ADD COLUMN expired_at DATETIME");
    }
    if (!sessionCols.some(row => row.name === 'updated_at')) {
      console.log('[DB] Migration: Adding updated_at to sessions');
      await run("ALTER TABLE sessions ADD COLUMN updated_at DATETIME");
    }
  } catch (e) {
    // Column likely exists
  }
  ...
  try {
    ...
    // Check pppoe_users for billing_profile_id and account_number
    ...
    if (!pppoeUserCols.some(row => row.name === 'account_number')) {
      console.log('[DB] Migration: Adding account_number to pppoe_users');
      await run("ALTER TABLE pppoe_users ADD COLUMN account_number TEXT");
    }
  } catch (e) {
