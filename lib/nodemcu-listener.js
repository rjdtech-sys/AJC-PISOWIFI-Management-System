...
class NodeMCUListener {
  ...
  setupRoutes() {
    ...
    this.app.post('/api/nodemcu/pulse', express.json(), async (req, res) => {
      try {
        ...
        // Sync Sale to Cloud
        this.syncSaleToCloud(device, slotId, denomination);
  ...
  async syncDeviceToCloud(device) {
    if (!edgeSync.supabase || !edgeSync.machineId || !edgeSync.vendorId) {
        // Try to init if missing, but don't block too long
        if (!edgeSync.isInitialized) {
             // Just a check, don't await full init if it's slow, but here we probably should
             // If edgeSync isn't ready, we can't sync.
             return null; 
        }
        if (!edgeSync.supabase || !edgeSync.machineId || !edgeSync.vendorId) return null;
    }

    try {
        // First try to find by mac_address to get the ID
        let { data: existingDevice, error: findError } = await edgeSync.supabase
        ...
        if (cloudId) {
             ...
             // Insert new
             const { data: newDevice, error: insertError } = await edgeSync.supabase
                .from('nodemcu_devices')
                .insert({
                    vendor_id: edgeSync.vendorId,
                    machine_id: edgeSync.machineId,
