      ...
      */
      ...
      if (error) {
          ...
          if (error.message.includes('vendor_dashboard_realtime_vendor_id_fkey')) {
             ...
             try {
                 // Try to insert the missing realtime entry
                 ...
                 if (!insertError) {
                     ...
                     // Retry the sale insert immediately
                     ...
                     if (retryError) {
                         console.error('[EdgeSync] Retry failed after creating realtime entry:', retryError.message);
                         this.queueSync('sale', saleData);
                         return false;
                     }
                     
                     console.log('[EdgeSync] Sale recorded successfully after self-healing.');
                     return true;
                 } else {
             ...
             // If we failed to heal, queue it for later
             this.queueSync('sale', saleData);
             return false; 
          } else {
              throw error;
