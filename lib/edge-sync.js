          ...
          if (localSession) {
              ...
              if (remoteUpdated > localUpdated || topupDetected) {
                  ...
                  await db.run(
                      ...
                      [remoteDevice.remaining_seconds, remoteDevice.total_paid, remoteDevice.updated_at || new Date().toISOString(), remoteDevice.mac_address]
                  );
      ...
      try {
          ...
          if (data && data.remaining_seconds > 0) {
              ...
              if (existing) {
                  ...
                  await db.run(
                      'UPDATE sessions SET remaining_seconds = ?, total_paid = ?, updated_at = ? WHERE mac = ?',
                      [data.remaining_seconds, data.total_paid || 0, data.updated_at || new Date().toISOString(), mac]
                  );
              } else {
                   ...
                   try {
                       await db.run(
                           'INSERT INTO sessions (mac, remaining_seconds, total_paid, connected_at, updated_at, is_paused) VALUES (?, ?, ?, ?, ?, 0)',
                           [mac, data.remaining_seconds, data.total_paid || 0, new Date().toISOString(), data.updated_at || new Date().toISOString()]
                       );
                   } catch (insertErr) {
