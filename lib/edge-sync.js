  ...
  async recordSale(saleData) {
    ...
    try {
      ...
      try {
        ...
        if (!vendorRealtime) {
          console.log('[EdgeSync] Vendor not found in realtime table. Attempting to create entry...');
          // Try to insert with just vendor_id - if other columns are required, this might fail
          // but it's our best attempt to self-heal
          const { error: insertError } = await this.supabase
            .from('vendor_dashboard_realtime')
            .insert({ vendor_id: vendorId });
            
          if (insertError) {
             console.warn('[EdgeSync] Failed to create realtime entry:', insertError.message);
          } else {
             console.log('[EdgeSync] Created vendor_dashboard_realtime entry.');
          }
        }
      } catch (e) {
      ...
      if (error) {
          ...
          if (error.message.includes('vendor_dashboard_realtime_vendor_id_fkey')) {
             console.warn('[EdgeSync] Hit vendor FK error. Refreshing vendor ID from source...');
             
             // Force refresh vendor ID
             const { data: freshVendor, error: freshError } = await this.supabase
                .from('vendors')
                .select('vendor_id')
                .eq('id', this.machineId)
                .single();
                
             if (!freshError && freshVendor && freshVendor.vendor_id && freshVendor.vendor_id !== vendorId) {
                 console.log(`[EdgeSync] Vendor ID updated: ${vendorId} -> ${freshVendor.vendor_id}`);
                 this.vendorId = freshVendor.vendor_id;
                 this.saveLocalVendorId(freshVendor.vendor_id);
                 
                 // Retry insert with new vendor ID
                 const { error: retryError } = await this.supabase
                    .from('sales_logs')
                    .insert({
                      vendor_id: freshVendor.vendor_id,
                      machine_id: this.machineId,
                      amount: saleData.amount,
                      transaction_type: saleData.transaction_type || 'coin_insert',
                      created_at: new Date().toISOString(),
                 ...
                 throw error; // Throw original error if vendor ID didn't change
             }
          } else {
              throw error;
          }
