...
class EdgeSync {
  ...
  async syncClientsToCloud() {
      if (!this.supabase || !this.centralizedKey || !this.machineId) return;

      try {
          // Get active sessions from local DB
          const sessions = await db.all('SELECT mac, ip, token, remaining_seconds, connected_at FROM sessions WHERE remaining_seconds > 0');

          if (!sessions || sessions.length === 0) return;

          // Prepare payload for Supabase clients table
          const updates = sessions.map(session => ({
              mac_address: session.mac,
              session_token: session.token || `session-${session.mac}-${Date.now()}`,
              machine_id: this.machineId,
          ...
          if (error) console.error('[EdgeSync] Failed to sync clients:', error.message);
          // else console.log(`[EdgeSync] Synced ${updates.length} clients to cloud.`);
