      ...
      */
      ...
      if (error) {
          ...
          if (error.message.includes('vendor_dashboard_realtime_vendor_id_fkey')) {
             console.warn('[EdgeSync] Hit vendor FK error. Attempting to self-heal by unlinking invalid vendor...');
             
             // Try to unlink the machine from the invalid vendor
             try {
                 ...
                 if (!updateError) {
                     console.log('[EdgeSync] Successfully unlinked machine from invalid vendor. Clearing local state.');
                     this.vendorId = null;
                     this.saveLocalVendorId(null);
                     // We cannot proceed with recording sale as we have no vendor now.
                     // Queue it for later when machine is re-claimed.
                     this.queueSync('sale', saleData);
                     return false;
                 } else {
             ...
             // DO NOT throw error here, we have handled it by unlinking.
             // Throwing error causes the loop to continue and potentially retry indefinitely.
             return false; 
          } else {
              throw error;
          }
