...
class EdgeSync {
  ...
  async checkRoamingForToken(sessionToken, mac = null) {
      ...
      try {
          ...
          if (existing) {
              await db.run(
                  "UPDATE sessions SET remaining_seconds = ?, total_paid = ?, updated_at = ?, token = COALESCE(token, ?), ip = COALESCE(?, ip) WHERE mac = ?",
                  [data.remaining_seconds, data.total_paid || 0, data.updated_at || new Date().toISOString(), data.session_token || null, data.ip_address || null, targetMac]
              );
          } else {
              await db.run(
      ...
      try {
          ...
          if (localSession) {
              ...
              if (remoteUpdated > localUpdated || topupDetected) {
                  ...
                  await db.run(
                      "UPDATE sessions SET remaining_seconds = ?, total_paid = ?, updated_at = ?, token = COALESCE(token, ?) WHERE mac = ?",
                      [remoteDevice.remaining_seconds, remoteDevice.total_paid, remoteDevice.updated_at || new Date().toISOString(), remoteDevice.session_token || null, remoteDevice.mac_address]
                  );
                  try {
               ...
               for (const dev of devices) {
                   ...
                   if (local) {
                       ...
                       if ((remoteUpdated > localUpdated || topupDetected) && Math.abs(local.remaining_seconds - dev.remaining_seconds) > 30) {
                           await db.run(
                               "UPDATE sessions SET remaining_seconds = ?, total_paid = ?, updated_at = ?, token = COALESCE(token, ?) WHERE mac = ?",
                               [dev.remaining_seconds, dev.total_paid || 0, dev.updated_at, dev.session_token || null, dev.mac_address]
                           );
                           try {
      ...
      try {
          ...
          if (data && data.remaining_seconds > 0) {
              ...
              if (existing) {
                  ...
                  await db.run(
                      "UPDATE sessions SET remaining_seconds = ?, total_paid = ?, updated_at = ?, token = COALESCE(token, ?), ip = COALESCE(?, ip) WHERE mac = ?",
                      [data.remaining_seconds, data.total_paid || 0, data.updated_at || new Date().toISOString(), data.session_token || null, data.ip_address || null, mac]
                  );
                  try {
