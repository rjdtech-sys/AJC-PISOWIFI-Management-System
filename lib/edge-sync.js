...
const EDGE_SYNC_FETCH_TIMEOUT_MS = 15000;
...
class EdgeSync {
  constructor() {
    ...
    this._clientsSyncInFlight = false;
    this._roamingSyncInFlight = false;
    this._statusSyncFailures = 0;
    this._roamingSyncFailures = 0;
    this._nextStatusSyncAt = 0;
    this._nextRoamingSyncAt = 0;
  ...
  async checkRoamingForToken(sessionToken, mac = null) {
      if (!this.supabase || !sessionToken) return null;
      ...
      try {
          const { data, error } = await this.supabase
            .from('wifi_devices')
            .select('mac_address, ip_address, session_token, remaining_seconds, total_paid, updated_at')
          ...
          if (!data || !data.remaining_seconds || data.remaining_seconds <= 0) return null;

          const targetMac = (mac || data.mac_address || '').toUpperCase();
          if (!targetMac) return null;

          const existing = await db.get('SELECT mac, remaining_seconds, updated_at FROM sessions WHERE mac = ?', [targetMac]);
          if (existing) {
              await db.run(
                  "UPDATE sessions SET remaining_seconds = ?, total_paid = ?, updated_at = ?, token = CASE WHEN ? IS NULL OR ? = '' THEN token ELSE ? END, ip = COALESCE(?, ip) WHERE mac = ?",
                  [data.remaining_seconds, data.total_paid || 0, data.updated_at || new Date().toISOString(), data.session_token || null, data.session_token || null, data.session_token || null, data.ip_address || null, targetMac]
              );
          } else {
              await db.run(
                  'INSERT INTO sessions (mac, ip, remaining_seconds, total_paid, connected_at, updated_at, is_paused, token) VALUES (?, ?, ?, ?, ?, ?, 0, ?)',
                  [targetMac, data.ip_address || null, data.remaining_seconds, data.total_paid || 0, new Date().toISOString(), data.updated_at || new Date().toISOString(), data.session_token || null]
              );
          }

          try {
              const tok = data.session_token || sessionToken;
              if (tok) await db.run('DELETE FROM sessions WHERE token = ? AND mac != ?', [tok, targetMac]);
          } catch (e) {}

          console.log(`[EdgeSync] Restored roaming session by token for ${targetMac}: ${data.remaining_seconds}s`);
          return data;
      } catch (e) {
  ...
  async init() {
    ...
    const fetchWithTimeout = async (url, options = {}) => {
      ...
      const timeoutId = setTimeout(() => controller.abort(), EDGE_SYNC_FETCH_TIMEOUT_MS);

      const hasSignal = options && Object.prototype.hasOwnProperty.call(options, 'signal') && options.signal;
      const finalOptions = hasSignal ? options : { ...options, signal: controller.signal };

      try {
        return await baseFetch(url, finalOptions);
      } finally {
  ...
  startStatusSync() {
    ...
    console.log(`[EdgeSync] Status sync started (every ${Math.round(STATUS_SYNC_INTERVAL / 1000)}s)`);
  ...
  async syncMachineStatus(status) {
    if (this._statusSyncInFlight) return false;
    const now = Date.now();
    if (this._nextStatusSyncAt && now < this._nextStatusSyncAt) return false;
    ...
    try {
      ...
      this._statusSyncFailures = Math.min(this._statusSyncFailures + 1, 10);
      const delayMs = Math.min(5 * 60 * 1000, Math.max(STATUS_SYNC_INTERVAL, 1000 * Math.pow(2, this._statusSyncFailures)));
      this._nextStatusSyncAt = Date.now() + delayMs;
      console.error('[EdgeSync] Error syncing status:', summarizeError(err), `(backoff ${Math.round(delayMs / 1000)}s)`);
      return false;
    } finally {
      this._statusSyncInFlight = false;
    }
  ...
  async upsertCloudWifiDeviceKeepingToken(payload) {
      if (!this.supabase || !payload || !payload.session_token) return null;

      const token = payload.session_token;
      const updateFields = { ...payload };
      delete updateFields.session_token;

      try {
          const { data: existing, error: selError } = await this.supabase
              .from('wifi_devices')
              .select('session_token, mac_address, machine_id')
          ...
          if (existing) {
              if (existing.mac_address !== payload.mac_address || existing.machine_id !== payload.machine_id) {
                  await this.supabase
                      .from('wifi_devices')
                      .delete()
              ...
              let { error: updError } = await this.supabase
                  .from('wifi_devices')
                  .update(updateFields)
                  .eq('session_token', token);

              if (updError && (updError.code === '23505' || String(updError.message || '').includes('unique constraint'))) {
                  await this.supabase
                      .from('wifi_devices')
                      .delete()
                      .eq('mac_address', payload.mac_address)
                  ...
                  ({ error: updError } = await this.supabase
                      .from('wifi_devices')
                      .update(updateFields)
                      .eq('session_token', token));
              }

              return updError || null;
          }

          let { error: upsertError } = await this.supabase
              .from('wifi_devices')
              .upsert(payload, { onConflict: 'session_token' });

          if (upsertError && (upsertError.code === '23505' || String(upsertError.message || '').includes('unique constraint'))) {
              await this.supabase
                  .from('wifi_devices')
                  .delete()
                  .eq('mac_address', payload.mac_address)
              ...
              ({ error: upsertError } = await this.supabase
                  .from('wifi_devices')
                  .upsert(payload, { onConflict: 'session_token' }));
          }
  ...
  async forceSyncClient(payload) {
      ...
      try {
          const err = await this.upsertCloudWifiDeviceKeepingToken(payload);
          if (err) console.error(`[EdgeSync] Final force sync failed for ${payload.mac_address}:`, summarizeError(err));
      } catch (e) {
  ...
  async syncClientsToCloud() {
      ...
      try {
          ...
          const activeCutoffMs = nowMs - 120000;
          ...
          const sessions = await db.all(`
            SELECT 
              ...
              wd.hostname, wd.custom_name, wd.ip as device_ip, wd.last_seen as device_last_seen
            FROM sessions s
            LEFT JOIN wifi_devices wd ON s.mac = wd.mac
            WHERE wd.last_seen >= ? AND (s.remaining_seconds > 0 OR s.updated_at > datetime('now', '-5 minutes'))
          `, [activeCutoffMs]);
          ...
          for (const session of sessions) {
              ...
              const localUpdatedMs = new Date(session.updated_at || 0).getTime();
              const payload = {
                  mac_address: session.mac,
                  session_token: sessionToken,
                  machine_id: this.machineId,
                  vendor_id: this.vendorId,
                  ip_address: session.ip || session.device_ip || null,
              ...
              const existing = bestByToken.get(sessionToken);
              const candScore = (payload.remaining_seconds || 0) * 1000 + localUpdatedMs;
              if (!existing || candScore >= existing.score) {
                  bestByToken.set(sessionToken, { payload, score: candScore });
              }
          }

          const updates = Array.from(bestByToken.values()).map(v => v.payload);
  ...
  async handleRoamingUpdate(remoteDevice) {
      ...
      try {
          const localWifiDevice = await db.get('SELECT mac FROM wifi_devices WHERE mac = ?', [remoteDevice.mac_address]);
          if (!localWifiDevice) return;
          ...
          if (localSession) {
              ...
              const localUpdated = new Date(localSession.updated_at || 0).getTime();
              const remoteUpdated = new Date(remoteDevice.updated_at).getTime();
              const topupDetected = (remoteDevice.remaining_seconds || 0) > (localSession.remaining_seconds || 0) + 30;
              
              if (remoteUpdated > localUpdated || topupDetected) {
                  console.log(`[EdgeSync] Roaming update for ${remoteDevice.mac_address}: ${remoteDevice.remaining_seconds}s remaining`);
                  
                  await db.run(
                      "UPDATE sessions SET remaining_seconds = ?, total_paid = ?, updated_at = ?, token = CASE WHEN ? IS NULL OR ? = '' THEN token ELSE ? END WHERE mac = ?",
                      [remoteDevice.remaining_seconds, remoteDevice.total_paid, remoteDevice.updated_at || new Date().toISOString(), remoteDevice.session_token || null, remoteDevice.session_token || null, remoteDevice.session_token || null, remoteDevice.mac_address]
                  );
                  try {
                      if (remoteDevice.session_token) {
                          await db.run('DELETE FROM sessions WHERE token = ? AND mac != ?', [remoteDevice.session_token, remoteDevice.mac_address]);
                      }
                  } catch (e) {}
          ...
          } else if (remoteDevice.remaining_seconds > 0) {
              // INSERT NEW DEVICE for realtime roaming
              console.log(`[EdgeSync] Realtime roaming: Adding new device ${remoteDevice.mac_address}`);
              try {
                  await db.run(
                      'INSERT INTO sessions (mac, ip, remaining_seconds, total_paid, connected_at, updated_at, is_paused, token) VALUES (?, ?, ?, ?, ?, ?, 0, ?)',
                      [
                          remoteDevice.mac_address, 
                          remoteDevice.ip_address || null,
                          remoteDevice.remaining_seconds, 
                          remoteDevice.total_paid || 0, 
                          new Date().toISOString(), 
                          remoteDevice.updated_at || new Date().toISOString(),
                          remoteDevice.session_token || null
                      ]
                  );
                  try {
                      if (remoteDevice.session_token) {
                  ...
                  console.error(`[EdgeSync] Error inserting realtime roaming device ${remoteDevice.mac_address}:`, insertErr.message);
              }
          }
      } catch (e) {
  ...
  async syncRoamingSessions() {
      ...
      if (this._roamingSyncInFlight) return;
      const now = Date.now();
      if (this._nextRoamingSyncAt && now < this._nextRoamingSyncAt) return;
      
      this._roamingSyncInFlight = true;
      try {
           ...
           if (devices && devices.length > 0) {
               for (const dev of devices) {
                   ...
                   const localWifiDevice = await db.get('SELECT mac FROM wifi_devices WHERE mac = ?', [dev.mac_address]);
                   if (!localWifiDevice) continue;
                   ...
                   if (local) {
                       ...
                       const topupDetected = (dev.remaining_seconds || 0) > (local.remaining_seconds || 0) + 30;
                       
                       if ((remoteUpdated > localUpdated || topupDetected) && Math.abs(local.remaining_seconds - dev.remaining_seconds) > 30) {
                           await db.run(
                               "UPDATE sessions SET remaining_seconds = ?, total_paid = ?, updated_at = ?, token = CASE WHEN ? IS NULL OR ? = '' THEN token ELSE ? END WHERE mac = ?",
                               [dev.remaining_seconds, dev.total_paid || 0, dev.updated_at, dev.session_token || null, dev.session_token || null, dev.session_token || null, dev.mac_address]
                           );
                           try {
                               if (dev.session_token) await db.run('DELETE FROM sessions WHERE token = ? AND mac != ?', [dev.session_token, dev.mac_address]);
                           } catch (e) {}
                       }
                   } else {
                       // Device not found locally, insert it so it shows in Device Management
                       console.log(`[EdgeSync] Downloading roaming device from cloud: ${dev.mac_address}`);
                       try {
                           await db.run(
                                'INSERT INTO sessions (mac, ip, remaining_seconds, total_paid, connected_at, updated_at, is_paused, token) VALUES (?, ?, ?, ?, ?, ?, 0, ?)',
                               [
                                   dev.mac_address, 
                                    dev.ip_address || null,
                                   dev.remaining_seconds, 
                                   dev.total_paid || 0, 
                                   new Date().toISOString(), 
                                    dev.updated_at || new Date().toISOString(),
                                    dev.session_token || null
                               ]
                           );
                           try {
                               if (dev.session_token) await db.run('DELETE FROM sessions WHERE token = ? AND mac != ?', [dev.session_token, dev.mac_address]);
                           ...
                           console.error(`[EdgeSync] Error inserting roaming device ${dev.mac_address}:`, insertErr.message);
                       }
                   }
               }
           }
          ...
          this._roamingSyncFailures = Math.min(this._roamingSyncFailures + 1, 10);
          const delayMs = Math.min(5 * 60 * 1000, Math.max(STATUS_SYNC_INTERVAL, 1000 * Math.pow(2, this._roamingSyncFailures)));
          this._nextRoamingSyncAt = Date.now() + delayMs;
          console.error('[EdgeSync] Error syncing roaming sessions:', summarizeError(e), `(backoff ${Math.round(delayMs / 1000)}s)`);
      } finally {
          this._roamingSyncInFlight = false;
      }
  }
  ...
  async checkRoamingForMac(mac) {
      ...
      try {
          ...
          if (data && data.remaining_seconds > 0) {
              ...
              const existing = await db.get('SELECT mac, remaining_seconds, updated_at FROM sessions WHERE mac = ?', [mac]);
              ...
              if (existing) {
                  ...
                  const topupDetected = (data.remaining_seconds || 0) > (existing.remaining_seconds || 0) + 30;
                  
                  if (!topupDetected && localUpdated >= remoteUpdated) {
                  ...
                  await db.run(
                      "UPDATE sessions SET remaining_seconds = ?, total_paid = ?, updated_at = ?, token = CASE WHEN ? IS NULL OR ? = '' THEN token ELSE ? END, ip = COALESCE(?, ip) WHERE mac = ?",
                      [data.remaining_seconds, data.total_paid || 0, data.updated_at || new Date().toISOString(), data.session_token || null, data.session_token || null, data.session_token || null, data.ip_address || null, mac]
                  );
                  try {
                      if (data.session_token) await db.run('DELETE FROM sessions WHERE token = ? AND mac != ?', [data.session_token, mac]);
                  } catch (e) {}
              } else {
                   ...
                   try {
                       await db.run(
                           'INSERT INTO sessions (mac, ip, remaining_seconds, total_paid, connected_at, updated_at, is_paused, token) VALUES (?, ?, ?, ?, ?, ?, 0, ?)',
                           [mac, data.ip_address || null, data.remaining_seconds, data.total_paid || 0, new Date().toISOString(), data.updated_at || new Date().toISOString(), data.session_token || null]
                       );
                       try {
                           if (data.session_token) await db.run('DELETE FROM sessions WHERE token = ? AND mac != ?', [data.session_token, mac]);
                       } catch (e) {}
  ...
  async syncDeviceToCloud(mac, remainingSeconds, totalPaid = 0) {
      ...
      try {
          ...
          const error = await this.upsertCloudWifiDeviceKeepingToken(updatePayload);
          if (error) console.error('[EdgeSync] Failed to sync single device:', summarizeError(error));
          else console.log(`[EdgeSync] Synced device ${mac} to cloud: ${remainingSeconds}s`);
      } catch (e) {
