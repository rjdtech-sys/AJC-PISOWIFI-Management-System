...
// Status sync interval (30 seconds)
...
class EdgeSync {
  ...
  async syncClientsToCloud() {
      ...
      try {
          ...
          const sessions = await db.all('SELECT mac, ip, token, remaining_seconds, total_paid, connected_at FROM sessions WHERE remaining_seconds > 0');

          if (!sessions || sessions.length === 0) return;

          // 1. Fetch existing devices from Cloud to check for existing Session IDs
          // This ensures we respect existing cloud tokens instead of overwriting them
          const macs = sessions.map(s => s.mac);
          const { data: cloudDevices } = await this.supabase
            .from('wifi_devices')
            .select('mac_address, session_token')
          ...
          if (cloudDevices) {
              cloudDevices.forEach(d => {
                  if (d.mac_address && d.session_token) {
                      cloudTokenMap[d.mac_address] = d.session_token;
                  }
              });
          }
          ...
          for (const session of sessions) {
              ...
              const cloudToken = cloudTokenMap[session.mac];
              
              // If Cloud has a token and it's different (or local is missing), ADOPT Cloud token
              if (cloudToken && cloudToken !== sessionToken) {
                  // console.log(`[EdgeSync] Adopting existing Cloud Session Token for ${session.mac}: ${cloudToken}`);
                  sessionToken = cloudToken;
                  
                  // Update Local DB to match Cloud ("ibalik sa system")
                  try {
                      await db.run('UPDATE sessions SET token = ? WHERE mac = ?', [cloudToken, session.mac]);
                  } catch (updateErr) {
                      console.error('[EdgeSync] Failed to update local session token:', updateErr.message);
                  }
              }
              
              // Fallback if no token exists anywhere
              if (!sessionToken) {
                  sessionToken = `fallback-${this.machineId}-${session.mac}`;
              }
      ...
      try {
           ...
           if (devices && devices.length > 0) {
               for (const dev of devices) {
                   ...
                   const local = await db.get('SELECT remaining_seconds FROM sessions WHERE mac = ?', [dev.mac_address]);
                   if (local) {
                       // Update local if remote is different (simplified sync)
                       if (Math.abs(local.remaining_seconds - dev.remaining_seconds) > 30) {
                           await db.run(
                               'UPDATE sessions SET remaining_seconds = ? WHERE mac = ?', 
                               [dev.remaining_seconds, dev.mac_address]
                           );
                       }
                   }
               }
  ...
  async checkRoamingForMac(mac) {
      ...
      try {
          const { data, error } = await this.supabase
            .from('wifi_devices')
            .select('mac_address, remaining_seconds, total_paid')
            .eq('vendor_id', this.vendorId)
            .eq('mac_address', mac)
            .maybeSingle();
          ...
          if (data && data.remaining_seconds > 0) {
              ...
              const existing = await db.get('SELECT mac FROM sessions WHERE mac = ?', [mac]);
