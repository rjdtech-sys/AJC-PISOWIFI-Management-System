...
class EdgeSync {
  ...
  async checkRoamingForToken(sessionToken, mac = null) {
      ...
      try {
          ...
          const localWifiDevice = await db.get('SELECT mac FROM wifi_devices WHERE mac = ?', [targetMac]);
          if (!localWifiDevice) return null;
      ...
      try {
          ...
          if (error) {
             ...
             if (error.code === '23505' || error.message.includes('unique constraint')) {
                 ...
                 if (finalError) {
                     if (finalError.code === '23505' || finalError.message.includes('unique constraint')) {
                         const { error: updateByTokenError } = await this.supabase
                             .from('wifi_devices')
                             .update(payload)
                             .eq('session_token', payload.session_token);
                             
                         if (updateByTokenError) {
                             console.error(`[EdgeSync] Final force sync failed for ${payload.mac_address}:`, summarizeError(finalError));
                         }
                     } else {
                         console.error(`[EdgeSync] Final force sync failed for ${payload.mac_address}:`, summarizeError(finalError));
                     }
