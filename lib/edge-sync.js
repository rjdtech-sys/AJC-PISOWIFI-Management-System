...
let warnedEdgeDnsOffline = false;
...
class EdgeSync {
  ...
  async init() {
    ...
    try {
      ...
      if (isDnsResolutionError(err)) {
        if (!warnedEdgeDnsOffline) {
          console.warn('[EdgeSync] Cloud sync offline (DNS). Will retry automatically.');
          warnedEdgeDnsOffline = true;
        }
        if (!this.initRetryTimer) {
          const delay = this.initRetryMs;
          this.initRetryMs = Math.min(this.initRetryMs * 2, 300000);
      ...
      console.error('[EdgeSync] Failed to initialize machine identity:', err && err.message ? err.message : err);
    }
  }
  ...
  async syncMachineStatus(status) {
    ...
    try {
      ...
      if (isDnsResolutionError(err)) {
        if (!warnedEdgeDnsOffline) {
          console.warn('[EdgeSync] Cloud sync offline (DNS).');
          warnedEdgeDnsOffline = true;
        }
        return false;
      }
      console.error('[EdgeSync] Error syncing status:', err && err.message ? err.message : err);
      return false;
    }
  }
  ...
  async recordSale(saleData) {
    ...
    try {
      ...
      */
      ...
      if (isDnsResolutionError(err)) {
        if (!warnedEdgeDnsOffline) {
          console.warn('[EdgeSync] Cloud sync offline (DNS). Queueing sale.');
          warnedEdgeDnsOffline = true;
        }
        this.queueSync('sale', saleData);
        return false;
      }
      console.error('[EdgeSync] Error recording sale:', err && err.message ? err.message : err);
      this.queueSync('sale', saleData);
      return false;
    }
  }
