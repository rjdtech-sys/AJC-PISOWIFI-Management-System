...
class EdgeSync {
  constructor() {
    ...
    this.ignoredVendorId = null; // Store bad vendor IDs that cause FK errors
      ...
      try {
        ...
        if (vendorData && vendorData.vendor_id) {
          if (this.vendorId !== vendorData.vendor_id) {
            // Ignore bad vendor IDs that have been flagged locally
            if (this.ignoredVendorId && vendorData.vendor_id === this.ignoredVendorId) {
                console.warn(`[EdgeSync] Ignoring known invalid vendor ID: ${vendorData.vendor_id}. Waiting for new claim.`);
                // If current local vendorId is same as ignored, clear it
                if (this.vendorId === this.ignoredVendorId) {
                    this.vendorId = null;
  ...
  async recordSale(saleData) {
    ...
    try {
      ...
      */
      ...
      if (error) {
          // If error is FK constraint on vendor_dashboard_realtime, try to force refresh vendorId and retry
          if (error.message.includes('vendor_dashboard_realtime_vendor_id_fkey')) {
             console.warn(`[EdgeSync] Hit vendor FK error with ID ${vendorId}. Marking as invalid/ignored.`);
             
             // 1. Mark this vendor ID as invalid locally so we don't pick it up again
             this.ignoredVendorId = vendorId;
             this.vendorId = null;
             this.saveLocalVendorId(null);

             // 2. Try to unlink the machine from the invalid vendor in cloud
             try {
                 const { error: updateError } = await this.supabase
                    .from('vendors')
                    .update({ vendor_id: null, status: 'offline' }) // Unlink and set offline
                    .eq('id', this.machineId);
                    
                 if (!updateError) {
                     console.log('[EdgeSync] Successfully unlinked machine from invalid vendor.');
                 } else {
                     console.error('[EdgeSync] Failed to unlink invalid vendor:', updateError.message);
                 }
             } catch (unlinkErr) {
                 console.error('[EdgeSync] Exception during unlink:', unlinkErr.message);
             }
             
             // 3. Re-queue the sale for later (when a VALID vendor claims it)
             this.queueSync('sale', saleData);
             
             // DO NOT throw error here, we have handled it.
             return false; 
          } else {
              throw error;
          }
