...
class EdgeSync {
  ...
  async forceSyncClient(payload) {
      ...
      try {
          ...
          if (delError) {
             console.error(`[EdgeSync] Force sync delete failed:`, summarizeError(delError));
          }
          ...
          if (error) {
             if (error.code === '23505' || error.message.includes('unique constraint')) {
                 const { error: updateByTokenError } = await this.supabase
                     .from('wifi_devices')
                     .update(payload)
                     .eq('session_token', payload.session_token);

                 if (!updateByTokenError) return;
                 ...
                 if (finalError) console.error(`[EdgeSync] Final force sync failed for ${payload.mac_address}:`, summarizeError(finalError));
             } else {
